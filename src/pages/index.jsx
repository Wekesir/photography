...
export default function index() {
  ...
  const handleSubmitMessage = async (e)=>{
    ...
    try {
      ...
      const {data} = await axios.post(`${BACKEND_SERVER}/guest/sendMessage.php`, messageFormInput)
      
      if(data?.status === 1){ 
        toast("Your message has been sent successfully! We will contact you shortly.")       
      } else {
        throw new Error(data.msg)
      }

    } catch (error) {
      toast(`Caught Error: ${error.message}` || "Unknown error trying to send message!");
    } finally {
      setMessageFormInput({})
      e.target.querySelector('button[type="submit"]').disabled = false 
          ...
          <div className="container-fluid content-wrapper">
              ...
              <div className="row">
                  ...
                  <div className="col-12 col-md-6 mb-2">
                      <form action="" className="py-3 px-5" onSubmit={handleSubmitMessage} method="post" style={{backgroundColor:'rgba(255,255,255,.1)'}}>
                        <div className="mb-3">
                          <label htmlFor="name" className="form-label text-white fw-bold">NAME</label>
                          <input type="text" onChange={handleFileInputChange} value={ messageFormInput.name || "" } name="name" className="form-control text-white bg-dark bg-opacity-75 border border-secondary" id="name" required/>
                        </div>
                        <div className="mb-3">
                          <label htmlFor="email" className="form-label text-white fw-bold">EMAIL</label>
                          <input type="email" onChange={handleFileInputChange} value={ messageFormInput.email || "" } name="email" className="form-control text-white bg-dark bg-opacity-75 border border-secondary" id="email" required/>
                        </div>
                        <div className="mb-3">
        ...
        <section id="footer" className="bg-black">
          <div className="container-fluid p-4">
            <div className="row g-3">
              <div className="col-12 col-md-3">
                <h5 className="text-white fw-bold">LYRICS STUDIOS</h5> <hr className="border border-secondary" />
                <p style={{color:'rgba(255,255,255,.9)'}}>
                  Lyrics Studios is more than just a photography studio—it’s where moments come alive. 
                  Our lens captures the poetry in everyday life, turning it into visual symphonies. 
                  From weddings to corporate events, family portraits to cinematic videos, we blend artistry with technology. 
                  With Lyrics Studios, your memories become timeless melodie!
                </p>
              </div>
              <div className="col-12 col-md-3">
                <h5 className="text-white fw-bold">QUICK ACCESS</h5>  <hr className="border border-secondary" />
                <p className="text-white" style={{color:'rgba(255,255,255,.9)'}}>
                  Get in touch with us through our various social media plaforms. For the best photography services today!
                </p>
              </div>
              <div className="col-12 col-md-3">
                <h5 className="text-white fw-bold">FOLLOW US</h5>  <hr className="border border-secondary" />
                <ul className="list-group gy-3" style={{color:'rgba(255,255,255,.9)'}}>
                  <Link to="#" className="list-group-item list-group-item-action bg-black text-white border-0"><i className="bi bi-facebook text-white me-2"></i>  Facebook</Link>
                  <Link to="#" className="list-group-item list-group-item-action bg-black text-white border-0"><i className="bi bi-instagram text-white me-2"></i>  Instagram</Link>
                  <Link to="#" className="list-group-item list-group-item-action bg-black text-white border-0"><i className="bi bi-twitter-x me-2"></i>  X</Link>
                  <Link to="#" className="list-group-item list-group-item-action bg-black text-white border-0"><i className="bi bi-youtube me-2"></i>  YouTube</Link>
                </ul>
              </div>
              <div className="col-12 col-md-3">
                <h5 className="fw-bold text-white">SUBSCRIPTIONS</h5> <hr className="border border-secondary" />
                ...
                <p className="text-center">
                  <Link to="/terms-conditions">Terms & Conditions</Link> | <Link to="/privacy-policy">Privacy Policy</Link>
                </p>            
              </div>
              <div className="col-12">
                <div className="container text-center py-3">
                  <p className="text-white">2024 Lyrics Photography. All rights reserved.</p>
                </div>
              </div>
            </div>
          </div>
